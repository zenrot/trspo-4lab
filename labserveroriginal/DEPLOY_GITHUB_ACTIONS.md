...
Для отправки писем на реальные почтовые ящики нужно добавить SMTP secrets в GitHub Actions:

- `SMTP_DOMAIN`: SMTP-сервер, например `smtp.gmail.com`
- `SMTP_EMAIL`: адрес отправителя
- `SMTP_USERNAME`: SMTP-логин
- `SMTP_PASSWORD`: SMTP-пароль или пароль приложения
- `SMTP_SSL`: `true` или `false`
- `SMTP_PORT`: обычно `587` для STARTTLS или `465` для SSL

Если эти secrets не заданы, LabServer отправляет письма в локальный контейнер `mailpit`. Его веб-интерфейс доступен по адресу:

```text
http://zenrot.root.sx:18083
```

Для Gmail, Yandex, Mail.ru и похожих провайдеров обычно нужен пароль приложения, а не обычный пароль от почтового ящика.
...
## Как проверить отправку писем на реальную почту

Проверить рабочий SMTP-конфиг на сервере:
...
Если там указан `mailpit`, значит реальные SMTP secrets не были добавлены или deploy еще не перезапускался после их добавления.

Если там указан внешний SMTP-сервер, можно проверять отправку:

1. Открыть админ-панель: `http://zenrot.root.sx:18080`.
2. Войти под администратором.
3. Открыть группу со студентом.
4. Убедиться, что email студента настоящий.
5. Нажать отправку учетных данных студенту.
6. Проверить реальный почтовый ящик студента.
...
## Как проверить отправку через Mailpit

Если внешний SMTP не настроен, открыть веб-интерфейс Mailpit:

```text
http://zenrot.root.sx:18083
```

Проверка письма через приложение:
...
Если письмо не пришло или не появилось в Mailpit:
...
3. Если используется Mailpit, проверить, что контейнер `mailpit` запущен:
...
Для Mailpit ожидаемые SMTP-настройки:
